...
#endif
    return mbed_open(name, openflags_to_posix(openflags));
}

extern "C" int mbed_open(const char *name, int oflag, ...)
{
    int fildes = reserve_filehandle();
    if (fildes < 0) {
        return fildes;
...
{
    return mbed_close(fh);
}

extern "C" int mbed_close(int fildes)
{
...
extern "C" int PREFIX(_write)(FILEHANDLE fh, const unsigned char *buffer, unsigned int length, int mode)
{
    ...
    if (convert_crlf(fh)) {
        ...
        for (ssize_t cur = 0; cur < slength; cur++) {
            if (buffer[cur] == '\n' && prev != '\r') {
                ...
                if (cur > written) {
                    r = mbed_write(fh, buffer + written, cur - written);
                    if (r < 0) {
                        return -1;
                    }
                    written += r;
    ...
    if (written < slength) {
        ssize_t r = mbed_write(fh, buffer + written, slength - written);
        if (r < 0) {
            return -1;
        }
        written += r;
...
extern "C" ssize_t mbed_write(int fildes, const void *buf, size_t length)
{
...
extern "C" int PREFIX(_read)(FILEHANDLE fh, unsigned char *buffer, unsigned int length, int mode)
{
    ...
    if (convert_crlf(fh)) {
        ...
        bytes_read = mbed_read(fh, buffer, length);
    }
...
extern "C" ssize_t mbed_read(int fildes, void *buf, size_t length)
{
...
extern "C" int mbed_poll(struct pollfd fds[], nfds_t nfds, int timeout)
{
    if (nfds > OPEN_MAX) {
        errno = EINVAL;
        return -1;
